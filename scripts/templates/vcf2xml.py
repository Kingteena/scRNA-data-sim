...
def parse_args():
    parser = argparse.ArgumentParser("Transform a multi-sample VCF file into an XML file")
    ...
    parser.add_argument("--filtering_density", type=float, default=0.0,
                        help="The requiered alignment density after filtering")
...
def vcf2xml(vcf, xml_template, output_xml, encoding="nd16", filtering_density=0.0):
    ...
    print("The initial snv position number was ", seq_length,  "with ", count_missing(sequences), " missing data.")
    ...
    seq_length=len(sequences[0])
    print("After filtering ", seq_length,  " snv positions remained with ", count_missing(sequences), " missing data.")
    ...
    for name, seq in zip(names, sequences):
        sequence_entries.append(
            f'\t\t<sequence id="seq_{name}" spec="Sequence" taxon="{name}" totalcount="16" value="{seq}"/>'
        )
    ...
    if re.search(pattern, xml_content, flags=re.DOTALL):
        ...
        raise ValueError('The XML template must contain a <data id="alignment"> section.')
...
if __name__ == "__main__":
    ...
    if not (0 <= args.filtering_density <= 1):
        sys.exit("Error: --filtering_density must be between 0 and 1 (inclusive).")
    vcf2xml(args.vcf, args.xml_template, args.output_xml, args.encoding, args.filtering_density)
